...
namespace ZC
{
    ...
    class Message
    {
        public byte[] Pack(ZCToVobcMessage ZcToVobcMes,int size)
        { 
            byte[] tempBytes = new byte[size];
            IntPtr structPtr = Marshal.AllocHGlobal(size);
            Marshal.StructureToPtr(ZcToVobcMes, structPtr, false);
            Marshal.Copy(structPtr, tempBytes, 0, size);
            Marshal.FreeHGlobal(structPtr);
            return tempBytes;
        }
       public  VobcToZCMessage UnPack(byte[] bytes)
        {
            int size = Marshal.SizeOf(typeof(VobcToZCMessage));
            //分配结构体内存空间
            IntPtr structPtr = Marshal.AllocHGlobal(size);
            Marshal.Copy(bytes, 0, structPtr, size);
            VobcToZCMessage VobcToZCMes;
            VobcToZCMes =(VobcToZCMessage) Marshal.PtrToStructure(structPtr, typeof(VobcToZCMessage));
            Marshal.FreeHGlobal(structPtr);
            return VobcToZCMes;
        }
    }
